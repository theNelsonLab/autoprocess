...
class TVIPSConverter:
    ...
    @staticmethod
    def convert_tvips_to_tif(tvips_file_path: str, pedestal: int, tif_name: str | None, 
                          raw_conversion: bool,
                          file_logger: logging.Logger, console_logger: logging.Logger) -> bool:
        """Convert .tvips movie to .tif frames. Returns True if all verifications pass."""
        try:
            # Get output directory path
            tvips_dir = Path(tvips_file_path).parent
            images_dir = tvips_dir / 'images'
            images_dir.mkdir(exist_ok=True)
            
            # Load .tvips with custom reader
            with TvipsReader(tvips_file_path) as tvips:
                frames = tvips.read_all_frames()

            base_name = tif_name if tif_name is not None else Path(tvips_file_path).stem

            # Log data type information
            TVIPSConverter.log_to_file(f"Original data type: {frames[0].dtype}", file_logger)
            ...
            TVIPSConverter.log_to_both(overall_stats_msg, file_logger, console_logger)
            ...
            for idx, frame_data in enumerate(frames):
                ...
                if raw_conversion:
                    ...
                    if not TVIPSConverter.check_data_range(frame_data, pedestal, file_logger):
                        all_frames_verified = False

                TVIPSConverter.log_to_file(
                ...
                TVIPSConverter.log_frame_statistics(frame_data, saved_tif, idx, base_name, file_logger)
                TVIPSConverter.log_to_file(f"Frame {idx+1} saved as {output_tif_path}", file_logger)
                
                if not TVIPSConverter.verify_conversion(frame_data, saved_tif, raw_conversion, pedestal, output_tif_path, file_logger):
                    all_frames_verified = False
            
            if all_frames_verified:
                TVIPSConverter.log_to_both("\nVerification passed for all frames", file_logger, console_logger)
            else:
            ...
            error_msg = f"Error processing {tvips_file_path}: {e}"
            TVIPSConverter.log_to_both(error_msg, file_logger, console_logger)
            return False
    ...
    def check_data_range(data: np.ndarray, pedestal: int, file_logger: logging.Logger) -> bool:
        ...
        if max_val > 65535:
            ...
            TVIPSConverter.log_to_file(warning_msg, file_logger)
            return False
        return True
    ...
    def log_frame_statistics(frame_data: np.ndarray, tif_data: np.ndarray, 
        ...
        TVIPSConverter.log_to_file(stats_msg, file_logger)
    ...
    def verify_conversion(original_data: np.ndarray, tif_data: np.ndarray,
        ...
        try:
            ...
            if np.array_equal(compare_original, compare_tif):
                TVIPSConverter.log_to_file(f'Verification passed for {tif_file_path}', file_logger)
                return True
            else:
            ...
            TVIPSConverter.log_to_file(f'Error during verification for {tif_file_path}: {e}', file_logger)
            return False
...
def main():
    ...
    try:
        ...
        # Setup split logging
        file_logger, console_logger = TVIPSConverter.setup_logging('ser_conversion.log', 'logs')
        
        folder_path = Path(args.folder)
        if not folder_path.exists():
            raise ValueError(f"The specified path does not exist: {folder_path}")
        
        TVIPSConverter.log_to_both(f"Processing folder: {folder_path}", file_logger, console_logger)
        if not args.raw:
            TVIPSConverter.log_to_both(f"Using pedestal value: {args.ped}", file_logger, console_logger)
        if args.tif_name:
            TVIPSConverter.log_to_both(f"Using custom TIF base name: {args.tif_name}", file_logger, console_logger)
        TVIPSConverter.log_to_both(f"Recursive search: {'enabled' if args.recursive else 'disabled'}", file_logger, console_logger)
        TVIPSConverter.log_to_both(f"Conversion mode: {'raw' if args.raw else 'processed'}", file_logger, console_logger)
        
        # Process SER files
        pattern = '**/*.ser' if args.recursive else '*.ser'
        ser_files = list(folder_path.glob(pattern))
        if not ser_files:
            TVIPSConverter.log_to_both("No .ser files were found in the specified folder", 
        ...
        for ser_file in ser_files:
            try:
                success = TVIPSConverter.convert_ser_to_tif(str(ser_file), args.ped,
                                                        args.tif_name, args.raw,
                                                        file_logger, console_logger)
                if not success:
                    all_files_success = False
            except Exception as e:
                TVIPSConverter.log_to_both(f"Failed to convert {ser_file.name}: {e}", 
                                       file_logger, console_logger)
                all_files_success = False
                continue
        ...
        if 'file_logger' in locals() and 'console_logger' in locals():
            TVIPSConverter.log_to_both(f"An error occurred: {e}", file_logger, console_logger)
        else:
